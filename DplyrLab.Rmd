...
```{r}
my_flights <- my_flights %>% mutate(t_changed_inflight = arr_delay - dep_delay)
```
...
```{r}
my_flights <- my_flights %>% arrange(desc(t_changed_inflight)) %>% select(t_changed_inflight, everything())
```
...
my_flights <- my_flights %>% 
  mutate(t_changed_inflight = arr_delay - dep_delay) %>% 
  arrange(desc(t_changed_inflight)) %>% 
  select(t_changed_inflight, everything())
```

5. Make a histogram of the amount of time gained using the `hist()` function

```{r}
hist(my_flights$t_changed_inflight, xlab='Time gained (+) or lost (-) in flight', ylab='Number of flights', main='Time Change during Flight', col='green')
...
```{r}
mean(my_flights$t_changed_inflight, na.rm=TRUE)
my_flights %>% summarize(mean=mean(t_changed_inflight, na.rm=TRUE))
# They lost on average about 5 and a half minutes
...
```{r}
Hobby_Airport <- my_flights %>% filter(dest == 'HOU') %>% select(origin, dest, t_changed_inflight)
Hobby_Airport
```
...
```{r}
my_flights %>% filter(dest == 'SEA') %>% select(dest, t_changed_inflight) %>% summarise(mean=mean(t_changed_inflight, na.rm=TRUE))
# Lost 11.7 minutes on average
```
...
my_flights %>% filter(origin == 'JFK' & dest == 'SEA') %>% 
  ...
  summarise(mean=mean(air_time, na.rm=TRUE), min=min(air_time, na.rm=TRUE), max=max(air_time,na.rm=TRUE))
...
starwars %>% na.omit(gender) %>% summarize(n=length(unique(gender)))
```
